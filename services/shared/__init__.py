...
def get_user_by_email(email: str) -> AuthUser | None:
    with db_cursor() as cur:
        cur.execute("SELECT id, email, role, display_name FROM users WHERE email=?", (email,))
        row = cur.fetchone()
        if not row:
            return None
        return AuthUser(id=row["id"], email=row["email"], role=row["role"], display_name=row.get("display_name"))


def get_user_by_id(user_id: str) -> AuthUser | None:
    with db_cursor() as cur:
        cur.execute("SELECT id, email, role, display_name FROM users WHERE id=?", (user_id,))
        row = cur.fetchone()
        if not row:
            return None
        return AuthUser(id=row["id"], email=row["email"], role=row["role"], display_name=row.get("display_name"))
...
def authenticate(email: str, password: str) -> AuthUser | None:
    with db_cursor() as cur:
        cur.execute("SELECT id, email, role, display_name, password_hash FROM users WHERE email=?", (email,))
        row = cur.fetchone()
        if not row:
            return None
