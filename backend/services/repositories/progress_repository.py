...
class ProgressRepository:
    ...
    def list_for_camp(
        self, learner_id: str, camp_id: str
    ) -> list[NodeProgress]:
        return list(
    ...
    def list_for_day(
        self, learner_id: str, camp_id: str, day: int
    ) -> list[NodeProgress]:
        return list(
            self.session.scalars(
                select(NodeProgress).where(
    ...
    def progress_map(
        ...
        for row in self.list_for_camp(learner_id, camp_id):
            out[(int(row.day), str(row.node_id))] = str(row.status)
        return out

    def day_passed_total(
        self, learner_id: str, camp_id: str, day: int
    ) -> tuple[int, int]:
        """Return (passed, total) unique nodes for a day, excluding unlock nodes."""
        latest: dict[str, str] = {}
        for row in self.list_for_day(learner_id, camp_id, day):
            nid = str(row.node_id)
            if nid.endswith("-unlock"):
                continue
            latest[nid] = str(row.status)
        total = len(latest)
        passed = sum(1 for st in latest.values() if st == "passed")
        return passed, total
