...
def _day_progress_counts(cur: Any, learner_id: str, camp_id: str, day: int) -> tuple[int, int]:
    """Return (passed, total) for a training day — unique nodes, excluding unlock."""
    cur.execute(
        """
        SELECT node_id, status FROM node_progress
    ...
    if not rows:
        return 0, 0
    # Prefer latest status per node when duplicate rows exist.
    latest: dict[str, str] = {}
    for r in rows:
        nid = str(r["node_id"])
        if nid.endswith("-unlock"):
            continue
        latest[nid] = str(r["status"])
    total = len(latest)
...
def learning_daily_summary(request: Request, day: int, camp_id: str | None = None) -> dict[str, Any]:
    ...
    with db_cursor() as cur:
        ...
        passed, total = _day_progress_counts(cur, learner_id, camp, day)
