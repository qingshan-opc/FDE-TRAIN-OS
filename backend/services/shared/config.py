...
def _repo_root() -> Path:
    """Resolve monorepo root whether `services/` is at repo root or under `backend/`.

    Prefer a tree that has frontend + scripts (local monorepo). Docker API images
    often omit ``web/``; fall back to the package parent that holds ``services/``
    (``/app`` or ``backend/``).
    """
    here = Path(__file__).resolve()
    for p in here.parents:
        if (p / "web").is_dir() and (p / "scripts").is_dir() and (
            (p / "class").is_dir() or (p / "contracts").is_dir()
        ):
            return p
    for p in here.parents:
        if (p / "services").is_dir() and (
            (p / "migrations").is_dir() or (p / "contracts").is_dir()
        ):
            return p
    # Legacy: services/shared/config.py → parents[2]
